...
export default async function AdminHomePage() {
  ...
  const [userCount, branchCount, packageCount, institutionCount, auditCount, templateCount, scheduleCount, countryCount] =
    await Promise.all([
      prisma.user.count(),
      prisma.branch.count(),
  ...
  const cards = [
    ...
    },
    {
      ...
      description: "Countries available in the backend pipeline.",
      count: countryCount,
    },
    {
      href: "/admin/settings",
      title: "Settings",
      description: "SLA thresholds and system configuration.",
      count: null,
    },
    {
        ...
        {cards.map((c) => (
          <Link key={c.href} href={c.href} className="card p-5 transition hover:shadow-md">
            <div className="flex items-start justify-between">
              ...
              {c.count != null && <span className="badge bg-slate-100 text-slate-600">{c.count}</span>}
            </div>
